...
class _ProfileScreenState extends State<ProfileScreen> {
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (!_isEditing) {
            ...
            _bloodController.text = userData['blood'] ?? '';
          ...
          return _isEditing 
              ...
              : _buildProfileView(user?.uid, userData, theme, isDark);
        },
      ),
    );
  }

  Widget _buildProfileView(String? uid, Map<String, dynamic> data, ThemeData theme, bool isDark) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
          checkupCount = snapshot.data!.docs.length;
          double totalTemp = 0;
          for (var doc in snapshot.data!.docs) {
            final d = doc.data() as Map<String, dynamic>;
            totalTemp += (d['Temp'] ?? 0);
            int hr = d['HeartRate'] ?? 0;
            if (hr > maxHR) maxHR = hr;
          }
          avgTemp = totalTemp / checkupCount;
        }
            ...
            children: [
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    _buildSectionHeader('BIOMETRICS', isDark),
                    ...
                    _buildSectionHeader('HEALTH STATISTICS', isDark),
                    const SizedBox(height: 16),
                    Container(
                      ...
                      child: Column(
                        children: [
                          _buildHistoryStatRow('Total Check-ups', checkupCount.toString(), Icons.analytics_outlined, isDark),
                          const Divider(height: 32),
                          _buildHistoryStatRow('Avg Temperature', '${avgTemp.toStringAsFixed(1)}°C', Icons.thermostat_outlined, isDark),
                          const Divider(height: 32),
                          _buildHistoryStatRow('Peak Heart Rate', '$maxHR BPM', Icons.favorite_outline, isDark),
                        ],
                      ),
                    ),
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'BASIC INFORMATION',
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: isDark ? Colors.grey[400] : Colors.grey, letterSpacing: 1.1),
                  ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildImageStyledTextField(_heightController, "175", isDark, keyboardType: TextInputType.number, digitsOnly: true),
                          ],
                        ),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildImageStyledTextField(_weightController, '70', isDark, keyboardType: TextInputType.number, digitsOnly: true),
                          ],
                        ),
                      ),
                    ],
  ...
  Widget _buildHistoryStatRow(String label, String value, IconData icon, bool isDark) {
    return Row(
      children: [
        Icon(icon, size: 20, color: isDark ? Colors.white70 : const Color(0xFF1A1A1A)),
        const SizedBox(width: 12),
        Expanded(
          child: Text(
            label,
            style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isDark ? Colors.white70 : Colors.black87),
          ),
        ),
        Text(
          value,
  ...
  Widget _buildStatCell(String label, String value, bool isDark) {
    return Column(
      ...
      children: [
        Text(
          label,
          style: TextStyle(fontSize: 11, color: isDark ? Colors.grey[400] : Colors.grey[600], fontWeight: FontWeight.bold, letterSpacing: 1.1),
        ),
        const SizedBox(height: 8),
        Text(
          value,
